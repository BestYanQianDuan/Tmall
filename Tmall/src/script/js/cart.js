...
(function() {
    class drawCart {
        ...
        saveAmount() {
                var $sum = 0;
                $('.item:visible').each(function(index, element) {
                    ...
                    success: function(data) {
                        $.each(data, function(index, value) {
                            if (id == value.sid) { //遍历判断sid和传入的sid是否相同，方便将那条数据设置到渲染的商品列表中。
                                ...
                                $clonebox.find(".f1 img").attr("src", value.picurl); //商品图片
                                $clonebox.find(".f1 img").attr("sid", value.sid);
                                $clonebox.find(".f2").html(value.title); //标题描述
                                $clonebox.find(".f3 span").html(value.price); //单价
                                $clonebox.find(".f4 input").val(count); //购买数量
                                $clonebox.find(".f5 span").html((value.price * count).toFixed(2)); //单品总额
                                $clonebox.css('display', 'block');
                                _this.$list.append($clonebox);
                            }
                        });
                        _this.compute(); //总计
                    }
        ...
        allSelect() {
                ...
                var allSelectTimer = setTimeout(function() {
                    $('.item:visible').find('.single').prop('checked', $("#all").prop('checked'));
                    _this.compute(); //总计
                    clearTimeout(allSelectTimer);
                }, 100);
                // 全选按钮
                $('#all').on("change", function() {
                    $('.item:visible').find('.single').prop('checked', $(this).prop('checked'));
                    _this.compute(); //总计
                });
                // 单个选项按钮
                this.$bookslist.on("change", ".single", function() { // $(this)指向.single
                    if ($('.item:visible').find('input:checkbox').length == $('.item:visible').find('input:checked').length) {
        ...
        bookCount() {
                ...
                $(".more").on("click", function() {
                    ...
                    $(this).parents('.item').find('.f5 span').html(_this.singlegoodsprice($(this))); //改变后的价格
        ...
        singlegoodsprice(obj) { //obj:当前元素
                var $dj = parseFloat(obj.parents('.item').find('.f3 span').html()); //单价
        ...
        setCookie(obj) {
                ...
                this.getCookie();
                // 重新设置
                var $index = obj.parents('.item').find('img').attr('sid'); //通过id找数量的位置
        ...
        delCurrentItem() {
                var _this = this;
                this.$list.on('click', '.f6 .del', function() {
                    _this.getCookie();
                    if (confirm('你确定要删除吗？')) {
